...
# ---------------------------------------------------------------------------
SOURCE = "https://www.youtube.com/watch?v=REPLACE_ME"   # <-- change this
LANGUAGE = "english"
...
CHUNK_OVERLAP = 100     # try: 50 (baseline) -> 100
RETRIEVAL_K = 6      # try: 4 (baseline) -> 6 -> 8
...
# WHY THIS CHANGED: MANUAL_GROUND_TRUTH used to be one flat dict, which
# meant every time you switched VIDEO_LABEL (v2 -> v3 -> v2...) you had to
# remember to manually paste in the matching ground-truth questions and
# swap them out -- easy to forget, and pasting the wrong video's ground
# truth silently produces meaningless similarity scores. Now each video's
# ground truth lives in its own entry, keyed by label, and the active one
# is picked automatically based on VIDEO_LABEL above.
GROUND_TRUTH_BY_VIDEO = {
    "v1": {
        "What was Henry traveling by?": "He was traveling by train.",
        "What did the man show Henry at the shooting range?": "A blonde woman named Bonnie, who never misses a shot.",
        "What happened to the rope bridge?": "The robbers cut the rope bridge when they saw Henry and Bonnie chasing them.",
        "Where did Henry and Bonnie finally find the robbers?": "Inside a mine, where the robbers were preparing explosives.",
    },
    "v2": {
...
MANUAL_GROUND_TRUTH = GROUND_TRUTH_BY_VIDEO.get(VIDEO_LABEL, {})
